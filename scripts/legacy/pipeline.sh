...
Pass-through options:
  --source-db <file>     Source DB path for calibration (default: <repo>/Cas-sources.txt).
  --solutions-dir <dir>  Solution directory (default: <repo>/outputs/solutions).
...
if [[ $RUN_AVERAGE -eq 1 ]]; then
  avg_cmd=("$SCRIPT_DIR/averaging.sh" --input-dir "$INPUT_DIR" --sun-sap "$SUN_SAP" --cal-sap "$CAL_SAP")
  if [[ -n "$OBSID" ]]; then
    avg_cmd+=(--obsid "$OBSID")
  fi
  "${avg_cmd[@]}"
...
if [[ $RUN_CALIBRATE -eq 1 ]]; then
  ...
  "$SCRIPT_DIR/calibration.sh" --cal-ms "$CAL_MS" --sun-ms "$SUN_MS" --source-db "$SOURCE_DB" --solutions-dir "$SOLUTIONS_DIR"
fi
...
if [[ $RUN_IMAGE -eq 1 ]]; then
  ...
  "$SCRIPT_DIR/wsclean_processing.sh" --ms "$SUN_MS" --type "$IMAGE_TYPE" --intervals-out "$INTERVALS_OUT"
fi
