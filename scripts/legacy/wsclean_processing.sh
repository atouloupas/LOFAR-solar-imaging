...
Options:
  ...
  --output-dir <dir>     Output root directory for FITS images (default: outputs/images_fits).
...
TYPE="calibrated"
INTERVALS_OUT=15
OUTPUT_DIR="outputs/images_fits"
NITER=5000
...
if [[ "$INTERVALS_OUT" -eq 1 ]]; then
  python3 "$REPO_ROOT/helioprojective_plot.py" "$OUTPUT_NAME-image.fits"
else
  python3 "$REPO_ROOT/reproject.py" "$OUTPUT_NAME"-t*-image.fits "outputs/images_png/$FILE_NAME/"
fi
